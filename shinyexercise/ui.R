...
shinyUI(fluidPage(
  ...
  titlePanel("Data Exploration Using Shiny"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      h3('Explore mtcars'),
      varSelectInput(inputId = "continuous_variable",
                     label = "Select Categorical Variable",
                     data = select(mtcars, -categorical_varibles),
                     selected = "mpg"),
      varSelectInput(inputId = "categorical_variable",
                     label = "Select Categorical Variable",
    ...
    ), #closing bracket for tabsetPanel
    position = "right"
  )
))
